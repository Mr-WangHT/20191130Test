...
# label
# Anchor 标签中文本的位置；background(bg) 背景色；foreground(fg) 前景色；borderwidth(bd) 边框宽度；width 标签宽度；
# height 标签高度；bitmap 标签中的位图；font 字体和大小；image 标签中的图片；justify 多行文本的对齐方式；
# text 标签中的文本，可以使用'\n'表示换行；textvariable 显示文本自动更新，与StringVar等配合着用
# place
# anchor: 组件对齐方式；x: 组件左上角的x坐标；y: 组件右上角的y坐标；relx: 组件相对于窗口的x坐标，应为0-1之间的小数；
# rely: 组件相对于窗口的y坐标，应为0-1之间的小数；width: 组件的宽度；heitht: 组件的高度；
...
btn=[]
for i in range(1,10):
    btn[i]=tkinter.Button(root,text = i,font = ('微软雅黑',20),fg = ('#4F4F4F'),bd = 0.5,command = lambda : pressNum(i))
    btn[i].place(x = (i-1)%3*70,y = 285+55*int((i-1)/3),width = 70,height = 55)
...
# anchor: 指定按钮上文本的位置；background(bg) 指定按钮的背景色；bitmap: 指定按钮上显示的位图；borderwidth(bd) 指定按钮边框的宽度；
# command: 指定按钮消息的回调函数；cursor: 指定鼠标移动到按钮上的指针样式；font: 指定按钮上文本的字体；
# foreground(fg)： 指定按钮的前景色；height: 指定按钮的高度；image: 指定按钮上显示的图片；state: 指定按钮的状态（disabled）；
# text: 指定按钮上显示的文本；width: 指定按钮的宽度 padx设置文本与按钮边框x的距离，还有pady;
# activeforeground： 按下时前景色；textvariable： 可变文本，与StringVar等配合着用
